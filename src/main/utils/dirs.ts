...
export function mihomoIpcPath(): string {
  if (process.platform === 'win32') {
    return '\\\\.\\pipe\\GUAR-Clash\\mihomo'
  }
  const { core = 'mihomo' } = getAppConfigSync()
  if (core === 'system') {
    return '/tmp/guar-clash-mihomo-external.sock'
  }
  if (!checkCorePermissionSync(core as 'mihomo' | 'mihomo-alpha')) {
    return '/tmp/guar-clash-mihomo-api-noperm.sock'
  }
  return '/tmp/guar-clash-mihomo-api.sock'
}
